...
export class ContratService {
  ...
  getContratBySlug(slug: string): Observable<any> {
  ...
  updateContrat(slug: string, data: any): Observable<any> {
    return this.http.post<any>(
      `${environment.apiUrl}contrats-update/${slug}`,
      data,
    );
  }
  ...
  toggleRappel(slug: string): Observable<any> {
    return this.http.post<any>(
      `${environment.apiUrl}contrats-toggle-rappel/${slug}`,
      {},
    );
  }
}
